...
async function upload() {
  ...
  const data = await api.uploadFile(inputField.value.files[0], (progressEvent: any) => {
...
<template>
  ...
  <div class="text-white">
    <div
      v-if="status === statusEnum.NoFile"
      @click="inputField?.click()"
      class="w-80 h-40 rounded bg-gradient-to-br from-cyan-500 to-indigo-500 flex flex-col place-items-center justify-center gap-1 hover:from-cyan-600 hover:to-indigo-600"
    >
    ...
    <div
      v-if="status === statusEnum.HasFile"
      class="w-80 h-40 rounded bg-gradient-to-br from-cyan-500 to-indigo-500 flex flex-col place-items-center justify-center gap-1"
    >
      <p class="max-w-[260px]">Upload "{{ filename }}"?</p>
      <button
        @click="upload()"
        class="bg-green-500 w-[260px] hover:bg-green-600 flex place-items-center justify-center rounded p-1"
      >
    ...
    <div
      v-if="status === statusEnum.Uploading"
      class="w-80 h-40 rounded bg-gradient-to-br from-cyan-500 to-indigo-500 flex flex-col place-items-center justify-center gap-1"
    >
      <p class="max-w-[260px]">Upload "{{ filename }}"?</p>
    ...
    <div
      v-if="status === statusEnum.Hosting"
      class="w-80 h-40 rounded bg-gradient-to-br from-cyan-500 to-indigo-500 flex flex-row place-items-center justify-center gap-1 p-1.5"
    >
      ...
      <div class="h-full aspect-square">
        <QrCode
          :value="hostname + '/download/' + fileId"
          class="rounded"
          type="image/png"
          :color="{}"
        ></QrCode>
